...
# Strip the per-profile `rustflags` key (the `profile-rustflags`-gated one).
# This is the only `rustflags = [` occurrence in the manifest.
sed -i '/^rustflags = \[/d' "$MANIFEST"
# Strip the `[unstable] profile-rustflags` opt-in block from the cargo config
# (it is the file's opening section; removing its two lines is sufficient).
